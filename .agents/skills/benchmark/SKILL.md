---
name: benchmark
description: Build and run JustReflectMe Google Benchmark performance measurements when changing or investigating StringHelper or FileNavigator behavior.
---

# Benchmark

Use this skill for performance-sensitive changes to `StringHelper`, `FileNavigator`, or
`benchmarks/StringAndNavigationBenchmarks.cpp`, and when a user asks for benchmark results or a
...
```sh
build/bin/JRMStringAndNavigationBenchmarks
```

List available benchmarks without measuring them:

```sh
build/bin/JRMStringAndNavigationBenchmarks --benchmark_list_tests
```

Run a focused group while iterating, for example navigation benchmarks:

```sh
build/bin/JRMStringAndNavigationBenchmarks --benchmark_filter='BM_(GoToLineStart|FindFirstWithLineLimit|GetLineNumberAndColumn)'
```
