---
name: jrm-docs
description: Use this skill whenever writing, updating, reviewing, or generating documentation for JustReflectMe (jrm) — the C++ codegen/reflection library. Covers writing/updating AsciiDoc pages for the Antora documentation site under docs/, autonomously synthesizing API reference content by reading source code and Doxygen-style comments directly (not a mechanical Doxygen dump), pulling every code sample in via tagged AsciiDoc includes instead of copy-pasting so examples stay in sync with the code, and reconciling docs with the current codebase, unit tests, benchmarks, README.md, AGENTS.md, and other project SKILL files when they're outdated or incomplete. Trigger this whenever the user asks to write or refresh jrm docs, update the jrm README, generate or update an API reference for jrm, add or edit an Antora page under docs/, or prepare content for the jrm docs site — even if they don't say "documentation" explicitly (e.g. "explain the codegen API in the docs", "the README is stale, fix it").
---

# JustReflectMe (jrm) Documentation Skill

Writes and maintains the AsciiDoc/Antora documentation for JustReflectMe, and
...
1. **AGENTS.md** — project conventions and workflows take precedence over
   everything else; this is the top-level statement of intent for the repo.
2. **Other project SKILL files** — process/tooling conventions already
   established for this repo.
3. **Unit tests** — ground truth for expected behavior, edge cases, and error
   handling. Partial reads are fine: skim test names and assertions for the
   area you're documenting rather than reading every test file end to end.
4. **Current source code** (headers/implementation) — ground truth for what
   the API actually does and its actual signatures, where tests don't already
   settle the question. Benchmarks fall under this tier too — ground truth
   for any performance claims; don't restate old numbers from the README
   without checking they still hold.
5. **README.md** — treat as a helpful but possibly-stale starting point, not
   a source to transcribe. Verify anything specific (API shapes, behavior,
   numbers) against 1–4 before it goes into the docs.
...
1. Locate the public API surface — the headers that make up jrm's public
...
5. Running `doxygen` to produce XML, and feeding it through a converter (e.g.
   ...
   It's not a shortcut that replaces reading the code, and jrm has no
